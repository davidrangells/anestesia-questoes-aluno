...
function mapErro(code: string) {
  switch (code) {
    case "sem_acesso":
      return "Seu acesso ainda não foi liberado. Se você já comprou, aguarde alguns minutos.";
    case "verificacao":
      return "Falha ao verificar seu acesso. Tente novamente.";
    default:
      return "";
  }
}

export default function LoginAlunoClient() {
  const router = useRouter();
  const sp = useSearchParams();

  const erro = sp.get("erro") || "";
  const erroMsg = useMemo(() => mapErro(erro), [erro]);
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="w-full max-w-md rounded-3xl border bg-white shadow-sm">
        <div className="p-6 border-b">
          ...
          <div className="text-sm text-slate-500">Entrar na área do aluno</div>
        </div>
        ...
        <form onSubmit={onLogin} className="p-6 space-y-4">
          ...
          <div className="text-xs text-slate-500">
            O acesso é liberado automaticamente após a compra (via Eduzz).
          </div>
        </form>
      </div>
    </div>
